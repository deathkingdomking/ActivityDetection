...
def download_video(video_id, download_path, video_format="mp4", log_file=None):
  ...
  download_cmd = ["youtube-dl", "https://youtube.com/watch?v={}".format(video_id), "-f", "best", "-f", "mp4", "--output", download_path, "--no-continue"]
  return_code = subprocess.call(
    ["youtube-dl", "https://youtube.com/watch?v={}".format(video_id), "-f", "best", "-f", "mp4", "--output", download_path, "--no-continue"], stderr=stderr)
  print ('donwloading cmd: %s' % ' '.join(download_cmd))
...
def cut_video(raw_video_path, slice_path, start, end):
  ...
  print ('cut video %s, slice %s from %ss to %ss' % (raw_video_path, slice_path, start, end))

  cmd = ["ffmpeg", "-loglevel", "quiet", "-i", raw_video_path, "-strict", "-2",
                                 "-ss", str(start), "-to", str(end), slice_path]
  print ('cut cmd: %s' % ' '.join(cmd))
...
def process_video(video_id, directory, start, end, video_format="mp4", compress=False, overwrite=False, log_file=None):
  ...
  print ("download path=%s, mkv_path=%s, slice_path=%s" % (download_path, mkv_download_path, slice_path))
